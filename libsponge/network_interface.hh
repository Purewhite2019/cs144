...
class NetworkInterface {
  private:
    static constexpr size_t TIMEOUT_ARP = 5000;
    static constexpr size_t LIVETIME_ARP = 30000;
    ...
    std::unordered_map<uint32_t, std::pair<std::optional<EthernetAddress>, size_t>> _map_ip_eth{};

    std::unordered_map<uint32_t, std::queue<InternetDatagram>> _queue_dgram{};

    void update(const uint32_t ip, const EthernetAddress &eaddr);

  public:
    //* \brief Construct a network interface with given Ethernet (network-access-layer) and IP (internet-layer)
    // addresses
